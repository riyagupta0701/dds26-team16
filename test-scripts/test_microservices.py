...
class TestMicroservices(unittest.TestCase):
    ...
    def test_order(self):
        ...
        # Create a new order (the previous one is in terminal 'failed' state)
        order2: dict = tu.create_order(user_id)
        order_id2: str = order2['order_id']
        add_item_response = tu.add_item_to_order(order_id2, item_id1, 1)
        self.assertTrue(tu.status_code_is_success(add_item_response))
        add_item_response = tu.add_item_to_order(order_id2, item_id2, 1)
        self.assertTrue(tu.status_code_is_success(add_item_response))

        checkout_response = tu.checkout_order(order_id2)
